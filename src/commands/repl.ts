...
import { header, hint } from "../ui/theme.js";
...
async function getPromptPrefix(): Promise<string> {
...
async function executeCommand(args: string[]): Promise<void> {
  ...
  try {
    switch (cmd) {
      case "init":
        if (!args[1]) { console.log(chalk.red("  Usage: init <project-name>")); return; }
      ...
      case "projects":
        ...
        break;
      case "use":
        if (!args[1]) { console.log(chalk.red("  Usage: use <project-name>")); return; }
      ...
      case "index":
        ...
        break;
      case "ask":
        if (!args[1]) { console.log(chalk.red("  Usage: ask <question>")); return; }
        await askCommand(args.slice(1).join(" "));
        break;
      case "plan": {
        if (!args[1]) { console.log(chalk.red("  Usage: plan <feature>")); return; }
      ...
      case "config": {
        ...
        await configCommand(opts as Parameters<typeof configCommand>[0]);
        break;
      }
      case "help":
        printHelp();
