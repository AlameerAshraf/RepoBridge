...
async function addSingleRepo(projectName: string, repoPathOrUrl: string): Promise<boolean> {
  let repoPath: string;
  let repoName: string;
  ...
  if (isGitUrl(repoPathOrUrl)) {
    ...
    if (await fs.pathExists(cloneDir)) {
      ...
      try { execSync(`git -C "${cloneDir}" pull --ff-only`, { stdio: "pipe" }); } catch {}
      repoPath = cloneDir;
    } else {
  ...
  const entry: RepoEntry = { name: repoName, path: repoPath, url, addedAt: new Date().toISOString() };
  ...
  try {
    ...
    spinner.succeed(
      `${repoLabel(repoName)} — ${index.fileTree.length} files, ${index.apiRoutes.length} routes, ${index.exports.length} exports`
    );
    return true;
  } catch (err) {
...
export async function addCommand(firstRepoPath?: string): Promise<void> {
  console.log(header());
  ...
  try {
    ...
    console.log(
      successBox(
        ...
        `Total: ${finalConfig.repos.length} repositories`,
        "Repos Ready"
      )
    );
