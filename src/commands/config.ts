...
export async function configCommand(options: {
  ...
  if (options.show || (!options.provider && !options.model && !options.apiKey && !options.baseUrl)) {
    const provider = (config.provider || "anthropic") as ProviderName;
    ...
    console.log(
      infoBox(
        ...
        `API Key:   ${hasKey}\n` +
        `Base URL:  ${config.baseUrl || chalk.dim("default")}`,
  ...
  // Validate provider
  if (options.provider) {
    if (!VALID_PROVIDERS.includes(options.provider as ProviderName)) {
    ...
    if (!options.model) {
      config.model = DEFAULT_MODELS[options.provider as ProviderName];
    }
  }
