...
import { requireActiveProject, getAllRepoIndexes, saveDebate, savePlan, listPlans, } from "../lib/storage.js";
...
export async function debateCommand(feature, existingPlan) {
    ...
    try {
        ...
        if (result.conflicts.length === 0) {
            ...
            // Stats box
            console.log(boxen(`${chalk.red.bold(String(high))} high   ${chalk.yellow.bold(String(med))} medium   ${chalk.dim(String(low))} low   ${chalk.dim("|")}   ${result.rounds.length} rounds   ${result.conflicts.length} total`, {
