...
export async function analyzeRepo(repoIndex, feature, allIndexes) {
    const provider = await getProvider();
...
export async function crossReference(feature, repoAnalyses, allIndexes) {
    const provider = await getProvider();
...
export async function deepDive(feature, conflicts, repoAnalyses, allIndexes) {
    const provider = await getProvider();
    const repoContext = buildRepoContext(allIndexes);
    const analysesText = repoAnalyses
        .map((a) => `## ${a.repo}\n${a.analysis}`)
