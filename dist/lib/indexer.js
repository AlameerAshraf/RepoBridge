...
export async function indexRepo(repoPath, repoName) {
...
async function findAndReadEnvExample(repoPath) {
...
async function findOpenApiSpec(repoPath) {
...
async function scanApiRoutes(repoPath, fileTree) {
    const routes = [];
    const sourceFiles = fileTree.filter((f) => /\.(ts|js|py|go|rb)$/.test(f) &&
...
async function scanExports(repoPath, fileTree) {
    const exports = [];
    const indexFiles = fileTree.filter((f) => /(?:index|main|mod)\.(ts|js)$/.test(f) || /src\/[^/]+\.(ts|js)$/.test(f));
...
function safeGrep(args, repoPath) {
    try {
...
async function scanEvents(repoPath) {
    const events = [];
    const result = safeGrep(["-roh", "--include=*.ts", "--include=*.js", "--include=*.py",
        "-E", "(emit|publish|subscribe|on)\\([\"'][a-zA-Z0-9_.:-]+[\"']\\)"], repoPath);
    ...
    const result2 = safeGrep(["-roh", "--include=*.ts", "--include=*.js", "--include=*.py",
        "-E", "(queue|topic|channel|exchange).*[\"'][a-zA-Z0-9_.:-]+[\"']"], repoPath);
    const queuePattern = /['"]([a-zA-Z0-9_.:-]+)['"]/g;
    while ((match = queuePattern.exec(result2)) !== null) {
        const val = match[1];
        if (val.length > 3 && !events.includes(val))
            events.push(val);
    }
    return events.slice(0, 50);
}
async function scanAuthPatterns(repoPath) {
    const patterns = [];
    const result = safeGrep(["-rl", "--include=*.ts", "--include=*.js", "--include=*.py",
        "-E", "(middleware|auth|jwt|oauth|bearer|passport|guard)"], repoPath);
